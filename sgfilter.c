// Based on the C code by Fredrik Jonsson. Ref: http://jonsson.eu/programs/cweb/sgfilter/
...
void log_printf(const char *function_name, const char *format, ...)
{
    ...
    time(&time0);
    lt = *localtime(&time0);
    sprintf(logentry, "%02u%02u%02u %02u:%02u:%02u.%03ld ", lt.tm_year - 100,
...
{
    int *v;
    if ((v = (int*) calloc((nh - nl + 2), sizeof (int))) == NULL) {
        return NULL;
    }
    return v - nl + 1;
}
...
{
    double *v;
    if ((v = (double*) calloc((nh - nl + 2), sizeof (double))) == NULL) {
        return NULL;
    }
    for (long k = nl; k <= nh; k++)
        v[k] = 0.0;
    return v - nl + 1;
}
...
double** dmatrix(long nrl, long nrh, long ncl, long nch)
{
    ...
    long ncol = (nch - ncl + 1);
    double **m;
    if ((m = (double**) malloc(((nrow + 1) * sizeof(double*)))) == NULL) {
        return NULL;
    }
    m += 1;
    m -= nrl;
    if ((m[nrl] = (double*) malloc(((nrow * ncol + 1) * sizeof(double)))) == NULL) {
        return NULL;
    }
    m[nrl] += 1;
    m[nrl] -= ncl;
